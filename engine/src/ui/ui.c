...
UIComponent create_ui_component (const AppState* state, const Uint32 max_rects, const Uint32 max_texts, const char* font_path, const float ptsize) {
    ...
    ui.fragment = load_shader (
        state->device, "shaders/ui.frag.spv",
...
static SDL_GPUTexture* ui_create_text_texture(const AppState* state, SDL_Surface* abgr) {
    ...
    SDL_GPUTransferBuffer* tbuf = SDL_CreateGPUTransferBuffer(state->device, &tbinfo);
    if (!tbuf) {
    ...
    SDL_GPUTextureRegion dst = {.texture = tex, .w = (Uint32)abgr->w, .h = (Uint32)abgr->h, .d = 1};
...
int draw_text(UIComponent* ui, const AppState* state, const char* utf8,
    ...
    SDL_Color color = {(Uint8)(r * 255.0f), (Uint8)(g * 255.0f),
                       (Uint8)(b * 255.0f), (Uint8)(a * 255.0f)};
