...
MeshComponent create_cylinder_mesh(
    float radius_top, float radius_bottom, float height, int radial_segments,
    ...
    if (radial_segments < 3 || height_segments < 1) {
        SDL_Log("Cylinder must have at least 3 radial segments and 1 height segment");
    ...
    for (int i = 1; i < height_segments; i++) {
        ...
        points[idx++] = (vec2){radius_bottom + frac * (radius_top - radius_bottom), -half_height + frac * height};
    }
    ...
    out_mesh = create_lathe_mesh(points, idx, radial_segments, theta_start, theta_length, device);
    free(points);
    return out_mesh;
}