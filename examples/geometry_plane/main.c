...
SDL_AppResult SDL_AppInit (void** appstate, int argc, char** argv) {
    ...
    // plane
    plane = create_entity ();
    MeshComponent plane_mesh =
        create_plane_mesh (1.0f, 1.0f, 1, 1, state->device);
    if (plane_mesh.vertex_buffer == NULL) return SDL_APP_FAILURE;
    add_mesh (plane, plane_mesh);
    // plane material
    MaterialComponent plane_material =
        create_phong_material ((vec3) {1.0f, 1.0f, 1.0f}, SIDE_DOUBLE, state);
    add_material (plane, plane_material);
    // plane transform
    add_transform (
        plane, (vec3) {0.0f, 0.0f, 0.0f}, (vec3) {0.0f, 0.0f, 0.0f},
        (vec3) {1.0f, 1.0f, 1.0f}
    );
