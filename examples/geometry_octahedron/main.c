...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char** argv) {
    ...
    // octahedron material
    MaterialComponent octahedron_material = create_phong_material ((vec3) {1.0f, 1.0f, 1.0f}, SIDE_FRONT, state);
    add_material (octahedron, octahedron_material);
    // octahedron transform
    add_transform(octahedron, (vec3) {0.0f, 0.0f, 0.0f}, (vec3){0.0f, 0.0f, 0.0f}, (vec3){1.0f, 1.0f, 1.0f});
...
SDL_AppResult SDL_AppIterate(void* appstate) {
    ...
    TransformComponent transform = *get_transform(octahedron);
    vec3 rotation = euler_from_quat(transform.rotation);
    rotation.x += 0.005f;
    rotation.z += 0.01f;
    transform.rotation = quat_from_euler(rotation);
    add_transform(octahedron, transform.position, rotation, transform.scale);
