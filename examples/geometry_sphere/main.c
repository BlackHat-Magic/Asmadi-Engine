...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char** argv) {
    ...
    // sphere transform
    add_transform(sphere, (vec3) {0.0f, 0.0f, 0.0f}, (vec3){0.0f, 0.0f, 0.0f}, (vec3){1.0f, 1.0f, 1.0f});
...
SDL_AppResult SDL_AppIterate(void* appstate) {
    ...
    TransformComponent transform = *get_transform(sphere);
    vec3 rotation = euler_from_quat(transform.rotation);
    rotation.x += 0.005f;
    rotation.z += 0.01f;
    transform.rotation = quat_from_euler(rotation);
    add_transform(sphere, transform.position, rotation, transform.scale);
