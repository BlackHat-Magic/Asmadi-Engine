...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char** argv) {
    ...
    // cone
    cone = create_entity();
    MeshComponent cone_mesh = create_cone_mesh (0.5f, 1.0f, 16, 1, false, 0.0f, 2.0f * (float) M_PI, state->device);
    if (cone_mesh.vertex_buffer == NULL) return SDL_APP_FAILURE;
    add_mesh(cone, cone_mesh);
    // cone material
    MaterialComponent cone_material = create_phong_material ((vec3) {1.0f, 1.0f, 1.0f}, SIDE_FRONT, state);
    add_material (cone, cone_material);
    // cone transform
    add_transform(cone, (vec3) {0.0f, 0.0f, 0.0f}, (vec3){0.0f, 0.0f, 0.0f}, (vec3){1.0f, 1.0f, 1.0f});
...
SDL_AppResult SDL_AppIterate(void* appstate) {
    ...
    TransformComponent transform = *get_transform(cone);
    vec3 rotation = euler_from_quat(transform.rotation);
    rotation.x += 0.005f;
    rotation.z += 0.01f;
    transform.rotation = quat_from_euler(rotation);
    add_transform(cone, transform.position, rotation, transform.scale);
