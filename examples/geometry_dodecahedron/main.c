...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char** argv) {
    ...
    MaterialComponent dodecahedron_material = create_phong_material ((vec3) {1.0f, 1.0f, 1.0f}, SIDE_FRONT, state);
    add_material (dodecahedron, dodecahedron_material);
    // dodecahedron transform
    add_transform(dodecahedron, (vec3) {0.0f, 0.0f, 0.0f}, (vec3){0.0f, 0.0f, 0.0f}, (vec3){1.0f, 1.0f, 1.0f});
...
SDL_AppResult SDL_AppIterate(void* appstate) {
    ...
    TransformComponent transform = *get_transform(dodecahedron);
    vec3 rotation = euler_from_quat(transform.rotation);
    rotation.x += 0.005f;
    rotation.z += 0.01f;
    transform.rotation = quat_from_euler(rotation);
    add_transform(dodecahedron, transform.position, rotation, transform.scale);
