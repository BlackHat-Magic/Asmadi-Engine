...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char** argv) {
    ...
    // ring
    ring = create_entity();
    MeshComponent ring_mesh = create_ring_mesh (0.25f, 0.5f, 16, 16, 0.0f, 2.0 * (float) M_PI, state->device);
    if (ring_mesh.vertex_buffer == NULL) return SDL_APP_FAILURE;
    add_mesh(ring, ring_mesh);
    // ring material
    MaterialComponent ring_material = create_phong_material ((vec3) {1.0f, 1.0f, 1.0f}, SIDE_DOUBLE, state);
    add_material (ring, ring_material);
    // ring transform
    add_transform(ring, (vec3) {0.0f, 0.0f, 0.0f}, (vec3){0.0f, 0.0f, 0.0f}, (vec3){1.0f, 1.0f, 1.0f});
...
SDL_AppResult SDL_AppIterate(void* appstate) {
    ...
    TransformComponent transform = *get_transform(ring);
    vec3 rotation = euler_from_quat(transform.rotation);
    rotation.x += 0.005f;
    rotation.z += 0.01f;
    transform.rotation = quat_from_euler(rotation);
    add_transform(ring, transform.position, rotation, transform.scale);
